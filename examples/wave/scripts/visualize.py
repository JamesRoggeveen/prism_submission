...
mask = imread("data/taal_lake_mask_small.png")[:,:,2]

ny, nx = mask.shape

x_vec = jnp.linspace(-1,1,nx)
y_vec = jnp.linspace(-1,1,ny)
t_vec = jnp.linspace(-1,1,100)
...
c_slice = c_field.derivative(x_grid_2d, y_grid_2d, t_grid_slice, order=(0,0,1)).reshape(ny,nx)
c_slice = jnp.where(mask == 0, jnp.nan, c_slice)
cont = ax.contourf(x_grid_2d, y_grid_2d, c_slice, levels=100)
ax.set_xlabel('x')
ax.set_ylabel('y')
...
def update(frame):
    ...
    c_slice_new = c_field.derivative(x_grid_2d, y_grid_2d, t_grid_slice, order=(0,0,1))
    c_slice_new = c_slice_new.reshape(ny, nx)
    c_slice_new = jnp.where(mask == 0, jnp.nan, c_slice_new)

    cont = ax.contourf(x_grid_2d, y_grid_2d, c_slice_new, levels=100)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
