...
class SSAProblem(AbstractProblem):
    ...
    def equation_residual(self, problem_data: pr.data.CollocationPoints, config: ProblemConfig) -> jax.Array:
        ...
        mu = self.mu_field.evaluate(eq_col_x, eq_col_y)
        h = self.h_field.evaluate(eq_col_x, eq_col_y)
        mu_x = self.mu_field.derivative(eq_col_x, eq_col_y, order=(1,0))
        mu_y = self.mu_field.derivative(eq_col_x, eq_col_y, order=(0,1))

        h_x = self.h_field.derivative(eq_col_x, eq_col_y, order=(1,0))
        h_y = self.h_field.derivative(eq_col_x, eq_col_y, order=(0,1))
        u_x = self.u_field.derivative(eq_col_x, eq_col_y, order=(1,0))
        u_y = self.u_field.derivative(eq_col_x, eq_col_y, order=(0,1))
        v_x = self.v_field.derivative(eq_col_x, eq_col_y, order=(1,0))
        v_y = self.v_field.derivative(eq_col_x, eq_col_y, order=(0,1))
        u_xx = self.u_field.derivative(eq_col_x, eq_col_y, order=(2,0))
        u_yy = self.u_field.derivative(eq_col_x, eq_col_y, order=(0,2))
        u_xy = self.u_field.derivative(eq_col_x, eq_col_y, order=(1,1))
        v_xx = self.v_field.derivative(eq_col_x, eq_col_y, order=(2,0))
        v_yy = self.v_field.derivative(eq_col_x, eq_col_y, order=(0,2))
        v_xy = self.v_field.derivative(eq_col_x, eq_col_y, order=(1,1))
    ...
    def boundary_residual(self, problem_data: pr.data.BoundaryData, config: ProblemConfig) -> jax.Array:
        ...
        u_x = self.u_field.derivative(bc_col_x, bc_col_y, order=(1,0))
        u_y = self.u_field.derivative(bc_col_x, bc_col_y, order=(0,1))
        v_x = self.v_field.derivative(bc_col_x, bc_col_y, order=(1,0))
        v_y = self.v_field.derivative(bc_col_x, bc_col_y, order=(0,1))
        h = self.h_field.evaluate(bc_col_x, bc_col_y)
        mu = self.mu_field.evaluate(bc_col_x, bc_col_y)
        ...
        # f3 = 2*mu*(2*u_x + r*v_y)*n_x + mu*(r*u_y + v_x)*n_y -  0.5*h * n_x
        # f4 = 2*mu*(2*r*v_y + u_x)*n_y + mu*(r*u_y + v_x)*n_x - 0.5*h * n_y
        f3 = 2*mu*h*(2*u_x + r*v_y)- 0.5*h**2
        f4 = mu*(r*u_y + v_x)*h
...
def plot_fits(x_grid, y_grid, field, mask, ax):
    ny, nx = x_grid.shape
    field_eval = field.evaluate(x_grid, y_grid)
    field_eval = field_eval.reshape(ny, nx)
    # Reverse the direction of the y-axis in field_eval
    field_eval = field_eval[::-1, :]
    field_eval = jnp.where(mask, field_eval, jnp.nan)
    im = ax.imshow(field_eval, cmap="jet")
    return im
...
def load_and_sample_data(config):
    ...
    # eq_col = jax.random.uniform(subkey, (2,config.n_pde,), minval=-1, maxval=1)
    key, subkey = jax.random.split(rng_key)
    sampled_col_data = sample_from_data_dict(field_data["u"], config.n_pde, subkey)
    eq_col = sampled_col_data.coords
    collocation_data = pr.data.CollocationPoints(coords=(eq_col[0], eq_col[1]))

    bc_data = full_data_dict["bc"]
    bc_data = pr.data.BoundaryData(coords=(bc_data["x"][1::2], bc_data["y"][1::2]), normal_vector=(bc_data["nx"][1::2], bc_data["ny"][1::2]))
...
def initialize_fields(problem_data, basis, config):
    ...
    for key, data in reference_data.items():
        ...
        config["field_types"][key] = field.field_type
    return fields

def save_results(config, optimized_ssa,problem_data):
    ...
    field_dict = {
        ...
        "v": v_coeffs,
        "h": h_coeffs,
        "eq_col_x": problem_data["equation"].coords[0],
        "eq_col_y": problem_data["equation"].coords[1],
        "h_x": problem_data["reference_data"]["h"].coords[0],
        "h_y": problem_data["reference_data"]["h"].coords[1],
        "u_x": problem_data["reference_data"]["u"].coords[0],
        "u_y": problem_data["reference_data"]["u"].coords[1],
        "v_x": problem_data["reference_data"]["v"].coords[0],
        "v_y": problem_data["reference_data"]["v"].coords[1]
    }
...
if __name__ == "__main__":
    ...
    fields["mu_field"] = BasisField(basis, Coeffs.make_zero(basis.degs))
    config["field_types"]["mu"] = fields["mu_field"].field_type
    config["basis_type"] = basis.basis_type()
    ...
    optimized_ssa, _ = solver.solve(problem, problem_data, config, problem_config)
    end_time = time.time()
    print(f"Time taken: {end_time - start_time} seconds")
    config["solve_time"] = end_time - start_time
    save_results(config, optimized_ssa,problem_data)