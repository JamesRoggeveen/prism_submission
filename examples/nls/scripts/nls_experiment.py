...
def parse_args(config):
    ...
    config["results_dir"] = f"results/{timestamp}_nls"
    return config
...
if __name__ == "__main__":
    ...
    for N in N_list:
        basis = CosineChebyshevBasis2D((int(N), int(N)))
        config["basis_Nx"] = N
        config["basis_Nt"] = N
        ...
        end_time = time.time()
        print(f"Time taken {N}: {end_time - start_time} seconds",flush=True)
        ...
        u_l2_errors.append(u_l2_error)
        u_linf_errors.append(u_linf_error)
        v_l2_errors.append(v_l2_error)
        v_linf_errors.append(v_linf_error)
        ...
        h_l2_errors.append(h_l2_error)
        h_linf_errors.append(h_linf_error)

        print(f"L2 error {N}: h: {h_l2_error}, u: {u_l2_error}, v: {v_l2_error}",flush=True)
        print(f"Linf error {N}: h: {h_linf_error}, u: {u_linf_error}, v: {v_linf_error}",flush=True)
        times.append(end_time - start_time)
        config["time_taken"] = end_time - start_time
        config["script_name"] = f"{N}_nls"
        save_results(config, optimized_nls)

    N_list = jnp.array(N_list)
    h_l2_errors = jnp.array(h_l2_errors)
    h_linf_errors = jnp.array(h_linf_errors)
    times = jnp.array(times)
    u_l2_errors = jnp.array(u_l2_errors)
    u_linf_errors = jnp.array(u_linf_errors)
    v_l2_errors = jnp.array(v_l2_errors)
    v_linf_errors = jnp.array(v_linf_errors)
    error_data = jnp.vstack((N_list, h_l2_errors, h_linf_errors, times, u_l2_errors, u_linf_errors, v_l2_errors, v_linf_errors))
    ...
    ax[2].semilogy(N_list,h_l2_errors,marker="o",label="L2")
    ax[2].semilogy(N_list,h_linf_errors,marker="o",label="Linf")
    ax[2].set_xlabel("Basis Size")
    ax[2].set_ylabel("L2 Error")
    ax[2].legend()
    ax[3].semilogy(N_list,times,marker="o")
    ax[3].set_xlabel("Basis Size")
    ...
    h_linf_errors = jnp.array(h_linf_errors)
    times = jnp.array(times)
    error_data = jnp.vstack((N_list, h_l2_errors, h_linf_errors, times))
    jnp.save(f"{config['results_dir']}/errors.npy", error_data)