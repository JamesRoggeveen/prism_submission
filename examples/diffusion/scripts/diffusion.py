...
class DiffusionProblem(pr.AbstractProblem):
    ...
    @eqx.filter_jit
    def equation_residual(self, problem_data: pr.data.CollocationPoints, config: DiffusionConfig) -> jax.Array:
        ...
        return c_t - config.D * (c_xx + c_yy)

    def boundary_residual(self, problem_data: pr.data.BoundaryData, config: DiffusionConfig) -> jax.Array:
    ...
    @eqx.filter_jit
    def initial_condition_residual(self, problem_data: pr.data.ReferenceData, config: DiffusionConfig) -> jax.Array:
    ...
    @eqx.filter_jit
    def loss_function(self, problem_data: pr.ProblemData, config: DiffusionConfig) -> jax.Array:
        return jnp.sum(self.total_residual(problem_data, config)**2)

    @eqx.filter_jit
    def total_residual(self, problem_data: pr.ProblemData, config: DiffusionConfig) -> jax.Array:
        ...
        return jnp.concatenate([equation_weight*equation_residual/jnp.sqrt(n_equation), boundary_weight*boundary_residual/jnp.sqrt(n_boundary), ic_weight*ic_residual/jnp.sqrt(n_ic)])
...
def domain_from_png(config):
    img = imread(config.domain_path)[:,:,-1]
    mask = img > 0
    x_vec = jnp.linspace(-1,1,img.shape[0])
    y_vec = jnp.linspace(-1,1,img.shape[1])
...
if __name__ == "__main__":
    ...
    print("Begin domain from png", flush=True)
    mask_data, boundary_data = domain_from_png(config)
    print("Begin sampling data", flush=True)
    problem_data = sample_data(mask_data, boundary_data, config)
    print("Begin field initialization", flush=True)
    ...
    script_name = f"{timestamp}_diffusion"
    print(f"Script name: {script_name}", flush=True)
